						...
						<tr>
							...
							<td width="300"><c:choose>
									<c:when
										...
										<a
											href="recipe.do?command=recipeDetailWithoutView&rnum=${favoriteRecipeVO.rnum}">${favoriteRecipeVO.subject}
									...
									<c:otherwise>
										<a
											href="recipe.do?command=recipeDetailView&rnum=${favoriteRecipeVO.rnum}">${favoriteRecipeVO.subject}
											<c:if test="${replyCountList[status.index] !=0}">
												<span style="color: #445EDD; font-size: bold;">[${replyCountList[status.index]}]</span>
											</c:if>
										</a>
