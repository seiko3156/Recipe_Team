...
let recipeSaveReply=()=>{
	document.recipeReplyAddForm.action="recipe.do?command=addReply";
...
function ILikeThis( rnum ){
	let url = 'recipe.do?command=likeRecipe&rnum='+rnum;
	location.href=url;
}
...
function deleteThisReply(replyseq){
   ...
   if (msg==true){
      document.recipeReplyAddForm.action="recipe.do?command=deleteReply&replyseq="+replyseq;
      document.recipeReplyAddForm.submit();
   }else {
      return false;
   }
...
function footer_go_to( destination ){
	let url = 'recipe.do?command='+destination;
	location.href=url;
}
